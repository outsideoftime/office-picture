...
在 Word 和 Excel 中，选中已经插入的图片并双击即可预览；在 PowerPoint 中，请选中图片后右键单击，并在快捷菜单中选择“图片预览”。插件会弹出独立的大图预览窗口。图片来自当前 Office 文档中的选中对象，不需要本地文件路径或 URL。

## 当前功能

- Word：支持嵌入式图片和浮动图片。
- PowerPoint：支持幻灯片中的嵌入图片；选中图片后，通过右键菜单中的“图片预览”打开预览（不支持双击触发）。
...
- Word 和 Excel 的双击触发带有重入保护和关闭后的短暂去重，不会在关闭后再次弹出同一次预览。
...
4. 新建或打开文档，插入图片。
5. 在 Word 或 Excel 中，单击选中图片后双击图片；在 PowerPoint 中，选中图片后右键单击，并选择“图片预览”。确认出现“图片预览”弹窗。
